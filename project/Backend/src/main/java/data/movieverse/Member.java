...
public class Member {
	...
	private java.util.Set this_getSet (int key) {
		...
		else if (key == ORMConstants.KEY_MEMBER_MEDIA) {
			return ORM_media;
		}
	...
	public data.movieverse.Movie[] getMovies() {
		...
		for(java.util.Iterator lIter = movieMembers.getIterator();lIter.hasNext();) {
			lValues.add(((data.movieverse.MovieMember)lIter.next()).getMovie());
		}
	...
	public void addMovie(data.movieverse.MovieMember aMovieMember, data.movieverse.Movie aMovie) {
		aMovieMember.setMovie(aMovie);
		movieMembers.add(aMovieMember);
	}
	
	public data.movieverse.MovieMember getMovieMemberByMovie(data.movieverse.Movie aMovie) {
		data.movieverse.MovieMember[] lMovieMembers = movieMembers.toArray();
		for(int i = 0; i < lMovieMembers.length; i++) {
			if(lMovieMembers[i].getMovie().equals(aMovie)) {
	...
	public final data.movieverse.MovieMemberSetCollection movieMembers = new data.movieverse.MovieMemberSetCollection(this, _ormAdapter, ORMConstants.KEY_MEMBER_MOVIEMEMBERS, ORMConstants.KEY_MOVIEMEMBER_MEMBER, ORMConstants.KEY_MUL_ONE_TO_MANY);
	...
	public final data.movieverse.MediaSetCollection media = new data.movieverse.MediaSetCollection(this, _ormAdapter, ORMConstants.KEY_MEMBER_MEDIA, ORMConstants.KEY_MUL_ONE_TO_MANY);
