...
public class Badge {
	...
	public data.movieverse.MUser[] getmUsers() {
		...
		for(java.util.Iterator lIter = achievements.getIterator();lIter.hasNext();) {
			lValues.add(((data.movieverse.Achievement)lIter.next()).getmUser());
		}
	...
	public void addmUser(data.movieverse.Achievement aAchievement, data.movieverse.MUser amUser) {
		aAchievement.setmUser(amUser);
		achievements.add(aAchievement);
	}
	
	public data.movieverse.Achievement getAchievementBymUser(data.movieverse.MUser amUser) {
		data.movieverse.Achievement[] lAchievements = achievements.toArray();
		for(int i = 0; i < lAchievements.length; i++) {
			if(lAchievements[i].getmUser().equals(amUser)) {
	...
	public final data.movieverse.AchievementSetCollection achievements = new data.movieverse.AchievementSetCollection(this, _ormAdapter, ORMConstants.KEY_BADGE_ACHIEVEMENTS, ORMConstants.KEY_ACHIEVEMENT_BADGE, ORMConstants.KEY_MUL_ONE_TO_MANY);
