...
public class UserMovie {
	...
	private void this_setOwner(Object owner, int key) {
		if (key == ORMConstants.KEY_USERMOVIE_MUSER) {
			this.mUser = (data.movieverse.MUser) owner;
		}
		
		else if (key == ORMConstants.KEY_USERMOVIE_MOVIE) {
	...
	public void setmUser(data.movieverse.MUser value) {
		if (mUser != null) {
			mUser.userMovies.remove(this);
		}
		if (value != null) {
			value.userMovies.add(this);
		}
	}
